...
use ACA\WC\Export;
use ACA\WC\Settings\ShopOrder\Status;
use ACA\WC\Sorting;
use ACP;
use WC_Order;
...
class Orders extends AC\Column
	...
	public function get_raw_value( $user_id ) {
		return wc_get_orders( [
			'customer'       => $user_id,
			'status'         => $this->get_order_statuses() ?: 'any',
			'orderby'        => 'date',
			'order'          => 'DESC',
			'posts_per_page' => -1,
		] );
	...
	private function get_order_statuses() {
		$setting = $this->get_setting( Status::NAME );

		return $setting instanceof Status
	...
	public function register_settings() {
		$this->add_setting( new Status( $this ) )
