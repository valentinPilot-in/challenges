...
class Sales extends AC\Column {
	...
	public function get_raw_value( $variation_id ) {
		global $wpdb;

		$product = wc_get_product( $variation_id );

		$order_ids = ac_addon_wc_helper()->get_orders_ids_by_product_id( $product->get_parent_id(), apply_filters( 'acp/wc/column/product/sales/statuses', [ 'wc-completed' ], $this ) );
			...
			WHERE 
				meta_key = '_qty'
			 	...
			 	order_item_id IN (
					...
					WHERE wc_oim.meta_key = '_variation_id'
